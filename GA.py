...
class GA_Bound_A(): #Basic version of GA with Elitism
    ...
    def cal_Fitness_Value(self, chromo):
        ...
        self.eval_count += 1
        if self.eval_count % 100 == 0 and len(self.memo_opt) >= 1:
            self.convergence.append(self.memo_opt[-1])
        return obj
...
class GA_BoundAB(): #GA with PT
    ...
    def cal_Fitness_Value(self, chromo):
        ...
        obj = sourd.run()
        self.eval_count += 1
        if self.eval_count % 10 == 0 and self.eval_count > 0:
            self.convergence.append(self.memo_opt[-1])
...
class GA_PT_MS():  #GARPT
    ...
    def run(self):
        parent_gen = self.generate_Initial()
        while True:
            if len(self.memo_opt) > RESTART_THRESHOLD and self.memo_opt[-1] == self.memo_opt[-1 - RESTART_THRESHOLD]:
                parent_gen = self.generate_Initial()
