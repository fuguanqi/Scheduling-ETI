...
def opt_ET(jobs, para_due_dates, para_processing_times, para_earliness_penalties, para_tardiness_penalties):
    ...
    print("head last:", head_last)
    print("tail first:", tail_first)
    print("************** E/T Problem Solving Finished **************")

    return head_last, tail_first, tail_start


def dp(head_last, tail_first, uper_bound, jobs, b, para_due_dates, para_processing_times, para_earliness_penalties,
    ...
    if (head_last == tail_first - 1):
        ...
        block_start1, _, et_penalty1 = bt.time_block(jobs[:head_last + 1], para_due_dates, para_processing_times,
                                                     para_earliness_penalties, para_tardiness_penalties)
        block_start2, _, et_penalty2 = bt.time_block(jobs[tail_first:], para_due_dates, para_processing_times,
                                                     para_earliness_penalties, para_tardiness_penalties)
        if (merged_et_penalty <= et_penalty1 + et_penalty2 + b):
            ...
            eti_penalty = et_penalty1 + et_penalty2 + b
        ...
        block_lasts1, end_times1, eti_penalty1 = dp(head_last, tail_first - 1, uper_bound, jobs, b, para_due_dates,
        ...
        if (end_times1[n - 1] >= uper_bound):
            eti_penalty1 = utils.BIG_NUMBER
        if (end_times2[n - 1] >= uper_bound):
            eti_penalty2 = utils.BIG_NUMBER
...
def opt_ETI(uper_bound, jobs, b, para_due_dates, para_processing_times, para_earliness_penalties,
    ...
    end_times = list()
    eti_penalty = 0
    head_last, tail_first, tail_start = opt_ET(jobs, para_due_dates, para_processing_times, para_earliness_penalties,
                                               para_tardiness_penalties)
    if (head_last == n - 1):  # if the optimal schedule of ET problem has only one block
        block_start, _, et_penalty = bt.time_block(jobs, para_due_dates, para_processing_times,
                                                   para_earliness_penalties, para_tardiness_penalties)
        ...
        block_lasts, end_times, eti_penalty = dp(head_last, tail_first, uper_bound, jobs, b, para_due_dates,
                                                 para_processing_times, para_earliness_penalties,
                                                 para_tardiness_penalties)
